...
const ID = "ahrefs#keywords-explorer/overview";
const INPUT = {
    "queryParams": {
        "country": "us",
...
Deno.test(`${ID} happy (synthetic): 2 row(s) ⇒ max(50, 42 × 2) = 84 units`, async () => {
    ...
    assertEquals(result.usage, {
        "credits": { "default": 84 },
        "evidence": { "rows": 2, "minimum_top_up": 0 },
    });
});
...
Deno.test(`${ID} schema gate: bad inputs are rejected before the wire`, async () => {
    ...
    const fixture = await loadFixture(`${fixturesDir}synthetic-happy.json`);
    for (
        const bad of [{ "country": "us", "keywords": [] }] as Record<
            string,
            Json
        >[]
    ) {
