...
import { zSearchSuggestionsQueryParams } from "./schema/inputs.ts";

/**
 * GET /keywords-explorer/search-suggestions — Search Suggestions: 12 API units per row (the fixed field set below; design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Search Suggestions",
        summary:
            "Get autocomplete-style keyword suggestions for seeds, with volume and CPC.",
        description:
            "Get search-suggest (autocomplete) keyword ideas for seed " +
            "keywords, one row per suggestion. Returns the keyword, search " +
    ...
    request: { method: "GET", path: "/keywords-explorer/search-suggestions" },
    input: {
        schema: {
            // vendor defaults at the binding (D25); the row budget is REQUIRED — the estimate's whole basis
            queryParams: zSearchSuggestionsQueryParams.required({
                limit: true,
            }),
        },
    ...
    usage: {
         ...
         *  Rate card: 12 units/row = 2 × 1 + volume 10 — the field costs on
