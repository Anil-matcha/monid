...
import { zOrganicCompetitorsQueryParams } from "./schema/inputs.ts";

/**
 * GET /site-explorer/organic-competitors — Organic Competitors: 14 API units per row (the fixed field set below; design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Organic Competitors",
        summary:
            "Find domains competing with a target in organic search, with keyword overlap.",
        description:
            "Find the domains competing with a target for the same organic " +
            "keywords, one row per competitor. Returns the competitor " +
            "domain, its Domain Rating, the number of common keywords, the " +
            "keyword-overlap share, and the competitor's estimated traffic. " +
            "Supports a report date, country, target scope, filtering, " +
            "sorting, and a row budget via limit. Suited for competitive " +
            "landscape mapping.",
        docsUrl: "https://docs.ahrefs.com/",
        categories: ["seo"],
        notes: [
            "Billing: 14 API units per returned row, minimum 50 units per " +
    ...
    input: {
        schema: {
            ...
            queryParams: zOrganicCompetitorsQueryParams.extend({
                mode: zOrganicCompetitorsQueryParams.shape.mode.unwrap()
                    .default("subdomains"),
                protocol: zOrganicCompetitorsQueryParams.shape.protocol.unwrap()
                    .default("both"),
            }).required({ limit: true }),
        },
        ...
        toRequest: ({ data }) => ({
            ...
            queryParams: {
                ...
                select:
                    "competitor_domain,domain_rating,keywords_common,share," +
                    "traffic",
            },
        }),
    },
    usage: {
        /** The vendor formula `max(50, 14 × rows)` as two lines (design
         *  D1): rows at their per-row units, plus the top-up to the 50-unit
         *  request minimum. Both counts are the fns' job (D19).
         *  Rate card: 14 units/row = 4 × 1 + traffic 10 — the field costs on
         *  https://docs.ahrefs.com/en/api/reference/site-explorer/get-organic-competitors
        ...
        model: {
            ...
            components: {
                rows: {
                    ...
                    unit: Unit.RESULT,
                    label: "rows",
                    description: "returned rows, 14 API units each",
                    consumes: { credit: "default", amount: 14 },
                },
                minimum_top_up: {
                    kind: UsageModelKind.PER_UNIT,
                    unit: Unit.CREDIT,
