...
const ID = "ahrefs#site-explorer/organic-competitors";
const INPUT = {
    "queryParams": {
        "target": "ahrefs.com",
        "date": "2026-09-01",
...
Deno.test(`${ID} happy (synthetic): 3 row(s) ⇒ max(50, 14 × 3) = 50 units`, async () => {
    ...
    assertEquals(result.usage, {
        "credits": { "default": 50 },
        "evidence": { "rows": 3, "minimum_top_up": 8 },
    });
});
...
Deno.test(`${ID} schema gate: bad inputs are rejected before the wire`, async () => {
    ...
    for (
        const bad of [{
            ...
            "country": "us",
            "limit": 3,
            "where": '{"field":"not_a_field","is":["gt",100]}',
        }, {
            "target": "ahrefs.com",
            "date": "2026-09-01",
            "country": "us",
