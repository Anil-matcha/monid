...
import { zMetricsHistoryQueryParams } from "./schema/inputs.ts";

/**
 * GET /site-explorer/metrics-history — Traffic History: 21 API units per row (the fixed field set below; design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Traffic History",
        summary:
            "Track a target's organic and paid traffic estimates over time.",
        description:
            "Track a target's estimated organic and paid search traffic " +
            "between two dates. Returns the date, organic traffic, and paid " +
            "traffic per bucket. Supports target scope and an optional " +
            "country filter. Suited for traffic-trend charts and " +
            "before/after comparisons. One billed row per history_grouping " +
    ...
    request: { method: "GET", path: "/site-explorer/metrics-history" },
    input: {
        schema: {
            // vendor defaults at the binding (D25); date_to is REQUIRED — the hold counts buckets and a hook fn has no clock (D6)
            queryParams: zMetricsHistoryQueryParams.extend({
                mode: zMetricsHistoryQueryParams.shape.mode.unwrap().default(
                    "subdomains",
                ),
                protocol: zMetricsHistoryQueryParams.shape.protocol.unwrap()
                    .default("both"),
                history_grouping: zMetricsHistoryQueryParams.shape
