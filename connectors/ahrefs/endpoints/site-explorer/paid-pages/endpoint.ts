...
import { zPaidPagesQueryParams } from "./schema/inputs.ts";

/**
 * GET /site-explorer/paid-pages — Paid Landing Pages: 13 API units per row (the fixed field set below; design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Paid Landing Pages",
        summary:
            "List a target's paid-search landing pages with traffic and ad counts.",
        description:
            "List the landing pages a target runs search ads to, one row " +
            "per page. Returns the page URL, estimated monthly paid " +
            "traffic, the top paid keyword, and the number of ads pointing " +
    ...
    request: { method: "GET", path: "/site-explorer/paid-pages" },
    input: {
        schema: {
            // vendor defaults at the binding (D25); the row budget is REQUIRED — the estimate's whole basis
            queryParams: zPaidPagesQueryParams.extend({
                mode: zPaidPagesQueryParams.shape.mode.unwrap().default(
                    "subdomains",
                ),
                protocol: zPaidPagesQueryParams.shape.protocol.unwrap().default(
                    "both",
                ),
            }).required({ limit: true }),
        },
        ...
        toRequest: ({ data }) => ({
            ...
            queryParams: {
                ...
                select: "url,sum_traffic,top_keyword,ads_count",
            },
        }),
    },
    usage: {
        /** The vendor formula `max(50, 13 × rows)` as two lines (design
         *  D1): rows at their per-row units, plus the top-up to the 50-unit
         *  request minimum. Both counts are the fns' job (D19).
         *  Rate card: 13 units/row = 3 × 1 + sum_traffic 10 — the field costs
         *  on
        ...
        model: {
            ...
            components: {
                rows: {
                    ...
                    unit: Unit.RESULT,
                    label: "rows",
                    description: "returned rows, 13 API units each",
                    consumes: { credit: "default", amount: 13 },
                },
                minimum_top_up: {
                    kind: UsageModelKind.PER_UNIT,
                    unit: Unit.CREDIT,
