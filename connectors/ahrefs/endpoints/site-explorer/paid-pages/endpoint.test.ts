...
const ID = "ahrefs#site-explorer/paid-pages";
const INPUT = {
    "queryParams": { "target": "ahrefs.com", "date": "2026-09-01", "limit": 3 },
};

Deno.test(`${ID} happy (synthetic): 3 row(s) ⇒ max(50, 13 × 3) = 50 units`, async () => {
    ...
    assertEquals(result.usage, {
        "credits": { "default": 50 },
        "evidence": { "rows": 3, "minimum_top_up": 11 },
    });
});
...
Deno.test(`${ID} schema gate: bad inputs are rejected before the wire`, async () => {
    ...
    const fixture = await loadFixture(`${fixturesDir}synthetic-happy.json`);
    for (
        const bad of [{ "target": "ahrefs.com", "date": "2026-09-01" }, {
            "target": "ahrefs.com",
            "date": "2026-09-01",
            "limit": 101,
        }, {
