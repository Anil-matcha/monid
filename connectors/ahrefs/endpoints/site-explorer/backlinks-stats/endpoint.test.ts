...
const ID = "ahrefs#site-explorer/backlinks-stats";
const INPUT = {
    "queryParams": { "target": "ahrefs.com", "date": "2026-09-01" },
};

Deno.test(`${ID} happy (synthetic): 1 row(s) ⇒ max(50, 12 × 1) = 50 units`, async () => {
    ...
    assertEquals(result.usage, {
        "credits": { "default": 50 },
        "evidence": { "rows": 1, "minimum_top_up": 38 },
    });
});
...
Deno.test(`${ID} schema gate: bad inputs are rejected before the wire`, async () => {
    ...
    const fixture = await loadFixture(`${fixturesDir}synthetic-happy.json`);
    for (
        const bad of [{ "target": "ahrefs.com", "date": "2026-13-01" }, {
            "target": "ahrefs.com",
            "date": "2026-09-01",
            "select": "traffic",
