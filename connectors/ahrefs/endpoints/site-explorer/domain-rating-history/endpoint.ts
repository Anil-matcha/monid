...
export default defineEndpoint({
    meta: {
        ...
        summary:
            "Track a target's Domain Rating over time, one row per date bucket.",
        description:
            "Track how a domain's Domain Rating (0-100 link authority) " +
            "evolved between two dates. Returns the date and Domain Rating " +
            "per bucket. Suited for spotting authority gains and losses " +
            "over time. One billed row per history_grouping bucket (daily, " +
            "weekly, or monthly).",
    ...
    request: { method: "GET", path: "/site-explorer/domain-rating-history" },
    input: {
        schema: {
            // vendor defaults at the binding (D25); date_to is REQUIRED — the hold counts buckets and a hook fn has no clock (D6)
            queryParams: zDomainRatingHistoryQueryParams.extend({
                history_grouping: zDomainRatingHistoryQueryParams.shape
    ...
    usage: {
         ...
         *  costs on
         *  https://docs.ahrefs.com/en/api/reference/site-explorer/get-domain-rating-history
